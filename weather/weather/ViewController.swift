...
class ViewController: UIViewController {
    var WetherArray = [WeatherDetails]()
    
    private let  myimg:UIImageView   = {
        let img = UIImageView(image:UIImage( named: "dektop"))
    ...
    private let  mylogo:UIImageView   = {
        let img = UIImageView(image:UIImage( named: "loction"))
        img.contentMode = .scaleAspectFill
    ...
    override func viewDidLayoutSubviews() {
        ...
        mylogo.frame = CGRect(x: view.width/2 - 80, y: 30, width: 25, height: 30)
        mylabel.frame = CGRect(x: mylogo.right + 10, y: 30, width: 200, height: 30)
    ...
    func show(nme:String){
        WetherArray = Apihandler.shered.search_weather(with: nme)
        for  i in WetherArray {
